...
from utils import prediction_guard_llava_conv, encode_image, lvlm_inference_with_conversation
...
def get_default_rag_chain():
    ...
    # initialise an BridgeTower embedder 
    ...
    def prompt_processing(input):
        # get the retrieved results and user query
    ...
    # initialise prompt processing module as a Langchain RunnableLambda of function prompt_processing
...
class GradioInstance:
    ...
    def get_path_to_subvideos(self):
        if self.video_title is not None and self.path_to_img is not None:
            # ORIGINAL: info = video_helper_map[self.video_title]
...
<table style="border=0">
    <tr style="border:0">
        <td style="border:0">
            <p style="font-size:xx-large;font-family:Verdana, Arial, Helvetica, sans-serif;color: white;">
...
def http_bot(
    ...
    if len(all_images) == 0:
        # first query need to do RAG construct prompt
