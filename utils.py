...
def bt_embedding_from_prediction_guard(prompt, base64_image):
    ...
    # wait 1.5 seconds if the (free) API key is limitated to 1 request per second
    time.sleep(1.5)
...
def display_retrieved_results(results):
    ...
    for i, res in enumerate(results):
        ...
        print()
        img = Image.open(results[i].metadata['metadata']['extracted_frame_path'])
        img.show()
        print("------------------------------------------------------------")
...
def lvlm_inference_with_conversation(conversation, max_tokens: int = 200, temperature: float = 0.95, top_p: float = 0.1, top_k: int = 10):
    ...
    # wait 1.5 seconds if the (free) API key is limitated to 1 request per second
    time.sleep(1.5)
    # call chat completion endpoint at Prediction Guard
